...
const find = async ({ channelID, _id, url, category = "", regDay, minRecent, maxRecent, totalMinSale, totalMaxSale, minReview, maxReview, minPrice, maxPrice }) => {
  
  try {
    const userAgent = fakeUa()

    const bestcontent = await axios.get(`https://smartstore.naver.com/i/v1/smart-stores/${channelID}/best-products`,
      {
        headers: {
          "User-Agent":
          userAgent,
      ...
      try {
        ...
        if (
          ...
          if (content.data.seoInfo && content.data.seoInfo.sellerTags) {
            ...
            const originArea = content.data.originAreaInfo.content.includes("중국")
            ? "중국"
            : content.data.originAreaInfo.content.includes("일본")
            ? "일본"
